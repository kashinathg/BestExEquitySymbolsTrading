...
//  if ( !m_bStarted ) {
  if ( true ) {
    try {
      ...
      //  build list of options to monitor as well
      ...
      //  at this point, we have our own instrument names, the names to be used for
      ...
      //  take this code and put into module as it will be re-used not only in startup
...

      {
        ...
        //  broke this with IB symbol changes
...
void AppComboTrading::ConstructEquityPosition1a( const std::string& sName, pPortfolio_t pPortfolio, fAddPostion_t function ) {
  ...
  if ( im.Exists( sName, pInstrument ) ) {  // the call will supply instrument if it exists
    ...
    if ( bConstructed ) {
      ...
      //  need to fix this as needs different name sent
//      m_tws->RequestContractDetails(
//       	pInstrument,
...
//  problem with portfolio is 0 in m_EquityPositionCallbackInfo
...
void AppComboTrading::HandlePanelSymbolText( const std::string& sName ) {
  ...
  //  maybe need to disable this panel, as the order doesn't land in an appropriate portfolio or position.
  if ( m_bIBConnected ) {
    ou::tf::ib::TWS::Contract contract;
    ...
    //  needs to be redone if it is to be used.
